...
export const ManageProductDetails = () => {
	...
	const handleImagechange = (setImage) => {
		...
		fileInput.onchange = (e) => {
			...
			fileDataURLReader.onload = e => setImage(e.target.result);
...
const onAddAttributeClick = _ => setAttributes(prev => [...prev, { name: '', options: ['', ''] }]);
									...
									<Accordion.Item eventKey={ index } key={ index }>
										<Accordion.Header><CloseButton onClick={ () => removeAttribute(index) }/> 屬性 #{ index + 1}</Accordion.Header>
											<Accordion.Body>
												...
												<Row>
													<Col><Button type="button" onClick={ (e) => onAddOptionClick(e, index) }>增加選項</Button></Col>
												</Row>
										</Accordion.Body>
									</Accordion.Item>
								)) }
